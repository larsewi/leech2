...
"#,
    ...
    let patch_from1 = Patch::create(&hash1).unwrap();
    assert_eq!(patch_from1.num_blocks, 2);

    let sql_from1 = sql::patch_to_sql(&patch_from1).unwrap().unwrap();
    ...
    common::assert_wire_roundtrip(&patch_from1);

    // -- Patch from hash2 (just block 3) --
    let patch_from2 = Patch::create(&hash2).unwrap();
    assert_eq!(patch_from2.num_blocks, 1);

    let sql_from2 = sql::patch_to_sql(&patch_from2).unwrap().unwrap();
    ...
    common::assert_wire_roundtrip(&patch_from2);
}
