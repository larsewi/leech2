//! The `lch` and `libleech2` man pages.
//!
//! `lch.1` is rendered from the clap CLI definition so it never drifts from the
//! binary; the `libleech2` pages are produced by doxygen from the doc comments
//! in `include/leech2.h` so they never drift from the C API.
...
/// Regenerate every man page into `output_dir`, which is created if missing.
pub fn generate(repo_root: &Path, output_dir: &Path) -> Result<()> {
...
mod tests {
    ...
    fn generates_all_man_pages() -> Result<()> {
        ...
        let out = tempfile::tempdir()?;
        generate(crate::repo_root()?, out.path())?;
